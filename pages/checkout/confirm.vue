...
async function fetchStatus(phoneToUse = activePhone.value): Promise<void> {
  ...
  try {
    ...
    if (data.paymentStatus === 'paid' || data.paymentStatus === 'failed' || pollAttempts.value >= MAX_POLL_ATTEMPTS) {
      stopTimers();
    }
  } catch {
...
const whatsappHelpUrl = computed(() => {
  if (!orderData.value) {
    return buildWhatsAppLink('Hello The Sunrise Bookstore Concierge, I need assistance with my eBook order.');
  }
  ...
  const text = `Hello The Sunrise Bookstore Concierge, I need assistance with eBook Order #${orderRef}${paymentRef}. Customer: ${orderData.value.customerName}.`;
...
<template>
  <div class="min-h-screen flex flex-col bg-theme-canvas text-theme-ink antialiased">
    ...
    <main class="max-w-3xl mx-auto w-full py-8 sm:py-12 px-4 sm:px-6 flex-1">
      <div class="bg-theme-surface rounded-2xl shadow-card border border-theme-border p-6 sm:p-9 space-y-7">
        
        <!-- STATE A: PAYMENT PENDING -->
        <div v-if="isPaymentPending" class="space-y-6">
          <div v-if="isAutomatedSTK" class="bg-theme-surface-subtle border border-theme-accent-border rounded-2xl p-6 sm:p-7 space-y-4 shadow-soft">
            <div class="flex items-start gap-4">
              <div class="relative w-11 h-11 flex items-center justify-center flex-shrink-0 mt-0.5">
                <span class="absolute w-full h-full rounded-full bg-theme-accent/20 animate-ping" />
                <div class="w-10 h-10 rounded-full bg-theme-accent text-white flex items-center justify-center shadow-xs">
                  <Zap :size="20" />
                </div>
              </div>
              ...
              <div class="space-y-1.5 flex-1">
                <div class="flex items-center justify-between">
                  <span class="text-[10px] font-mono uppercase font-bold tracking-wider text-theme-accent">
                    Safaricom STK Prompt Sent
                  </span>
                  <span class="text-xs font-mono font-bold text-theme-ink bg-theme-surface px-2 py-0.5 rounded border border-theme-border">
                    {{ stkCountdown }}s
                  </span>
                </div>
                <h2 class="font-display font-bold text-base sm:text-lg text-theme-ink">
                ...
                <p class="text-xs sm:text-sm text-theme-muted leading-relaxed">
                  ...
                  <strong class="text-theme-ink font-mono">{{ orderData ? formatCurrency(orderData.total) : 'your order' }}</strong>.
                </p>
              </div>
            </div>

            <div class="pt-3 border-t border-theme-border flex flex-wrap items-center justify-between gap-3 text-xs">
              <div class="flex items-center gap-2 font-medium text-theme-muted">
          ...
          <div v-else-if="isDirectMpesa" class="bg-theme-surface-subtle border border-theme-accent-border rounded-2xl p-6 sm:p-7 space-y-4 shadow-soft">
            <div class="flex items-start gap-4">
              <div class="w-10 h-10 rounded-full bg-theme-accent text-white flex items-center justify-center shadow-xs flex-shrink-0">
                <Clock :size="20" />
              </div>
              <div class="space-y-1 flex-1">
                <span class="text-[10px] font-mono uppercase font-bold tracking-wider text-theme-accent block">
                ...
                <p class="text-xs sm:text-sm text-theme-muted leading-relaxed">
                  Thank you, <strong>{{ orderData?.customerName }}</strong>. Our team is validating your reference code against our till records.
                </p>
              </div>
            </div>

            <div class="bg-theme-surface rounded-xl p-4 border border-theme-border text-xs text-theme-ink space-y-2 shadow-2xs">
              <div class="flex justify-between items-center text-theme-muted">
                <span>Submitted Code:</span>
                <span class="font-mono font-bold text-theme-ink text-sm tracking-wider uppercase bg-theme-surface-subtle px-2.5 py-1 rounded border border-theme-border">
                  {{ orderData?.paymentReference || 'SUBMITTED' }}
                </span>
              </div>
              <div class="flex justify-between items-center text-theme-muted">
                <span>Total Amount:</span>
                <span class="font-mono font-bold text-theme-ink tabular-figure">
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 pt-1">
              <span class="text-[11px] text-theme-muted flex items-center gap-1.5 font-medium">
                <span class="w-2 h-2 rounded-full bg-theme-accent animate-pulse" />
                <span>eBook PDF downloads unlock automatically upon verification.</span>
              </span>
        ...
        <div v-else-if="isPaymentFailed" class="bg-theme-accent-soft border border-theme-accent-border rounded-2xl p-6 sm:p-7 space-y-4 shadow-soft">
          <div class="flex items-start gap-4">
            <AlertTriangle :size="26" class="text-theme-accent flex-shrink-0 mt-0.5" />
            <div class="space-y-1 flex-1">
              <span class="text-[10px] font-mono uppercase font-bold tracking-wider text-theme-accent block">
              ...
              <p class="text-xs sm:text-sm text-theme-muted leading-relaxed">
                The prompt was not approved. You can switch to another payment method or re-attempt with your phone number below without re-entering your order details.
              </p>
            </div>
          </div>
        ...
        <!-- RECOVERY DRAWER -->
        <div v-if="showPaymentRecovery" class="p-5 bg-theme-surface-subtle border border-theme-accent-border rounded-2xl space-y-4 shadow-soft animate-in fade-in">
          <div class="flex items-center justify-between border-b border-theme-border pb-2">
            <h3 class="font-display font-bold text-xs sm:text-sm text-theme-ink">
              Select Alternate Payment Method
            </h3>
            <button type="button" class="text-xs text-theme-muted underline cursor-pointer" @click="showPaymentRecovery = false">
              Close
            </button>
          </div>

          <div class="space-y-2.5 text-xs">
            <label class="flex items-center gap-2 cursor-pointer font-semibold text-theme-ink">
        ...
        <div v-if="isPaymentPaid || orderData?.status === 'confirmed' || orderData?.status === 'delivered'" class="space-y-6">
          <div class="text-center space-y-2 pb-5 border-b border-theme-border">
            <div class="w-13 h-13 bg-theme-accent-soft text-theme-accent rounded-full flex items-center justify-center mx-auto shadow-sm">
              <CheckCircle2 :size="28" class="text-theme-accent" />
            </div>
            <span class="text-[10px] font-mono font-bold uppercase tracking-widest text-theme-accent-hover bg-theme-accent-soft px-2.5 py-0.5 rounded-full border border-theme-accent-border">
              Payment Verified &amp; Approved
            </span>
            <h1 class="font-display text-2xl sm:text-3xl font-extrabold text-theme-ink">
              Your eBooks Are Ready!
            </h1>
            <p class="text-xs sm:text-sm text-theme-muted max-w-md mx-auto">
              Thank you, <strong>{{ orderData?.customerName }}</strong>. Your digital book license is ready for instant download.
            </p>
          </div>
          ...
          >
            <div class="w-9 h-9 rounded-full bg-theme-accent-soft text-theme-accent flex items-center justify-center flex-shrink-0">
              <MailCheck :size="18" />
            </div>
            <div>
              <strong class="font-semibold text-theme-ink block">Permanent Backup Dispatched via Email</strong>
          ...
          <div v-if="hasDigitalDownloads" class="space-y-3.5">
            <div class="flex items-center justify-between">
              <h3 class="font-display font-bold text-sm text-theme-ink uppercase tracking-wider flex items-center gap-2">
                <Download :size="16" class="text-theme-accent" /> Your Digital Editions (Direct Download)
              </h3>
              <span class="text-[10px] text-theme-accent-hover font-mono font-bold bg-theme-accent-soft px-2 py-0.5 rounded border border-theme-accent-border">
            ...
            <div class="grid sm:grid-cols-2 gap-3.5">
              ...
              >
                <div class="space-y-1">
                  <span class="text-[9px] font-mono font-extrabold uppercase px-2 py-0.5 rounded-md bg-theme-dark text-white">
                    {{ dl.format.toUpperCase() }} EBOOK
                  </span>
                  <h4 class="text-xs sm:text-sm font-bold text-theme-ink mt-1 line-clamp-1">{{ dl.bookTitle }}</h4>
                  <p class="text-[11px] text-theme-muted">Downloads remaining: {{ dl.maxDownloads - dl.downloadCount }}</p>
                </div>
          ...
          <!-- Order Summary -->
          <div class="border-t border-theme-border pt-4 space-y-2.5 text-xs">
            <div class="flex justify-between items-center text-theme-muted">
              <span>Order Reference</span>
              <span class="font-mono font-bold text-theme-ink">#{{ orderData?.orderId.slice(0, 8).toUpperCase() }}</span>
            </div>
            <div class="flex justify-between items-center text-theme-muted">
              <span>Payment Mode</span>
              <span class="font-mono font-bold text-theme-ink uppercase">
                {{ orderData?.paymentMethod === 'mpesa_manual' ? 'Direct M-Pesa Till' : 'Automated M-Pesa' }}
              </span>
            </div>
            <div v-if="orderData?.paymentReference" class="flex justify-between items-center text-theme-muted">
              <span>M-Pesa Reference</span>
              <span class="font-mono font-bold text-theme-ink uppercase bg-theme-surface-subtle px-2 py-0.5 rounded border border-theme-border">
                {{ orderData.paymentReference }}
              </span>
            </div>
            <div class="flex justify-between items-center text-theme-muted">
              <span>Delivery Format</span>
              <span class="font-medium text-theme-ink">Instant Cloudflare R2 Token Stream</span>
            </div>
            <div class="flex justify-between items-baseline pt-2.5 border-t border-theme-border text-sm font-bold text-theme-ink">
              <span>Total Amount</span>
              <span class="font-display font-extrabold text-theme-ink font-mono tabular-figure text-base sm:text-lg">
                {{ orderData ? formatCurrency(orderData.total) : '' }}
              </span>
            </div>
          </div>
          ...
          <div class="pt-2 flex flex-wrap gap-3.5">
            ...
            >
              <MessageCircle :size="15" class="text-[#25D366]" /> WhatsApp Concierge Desk
            </a>
            ...
            >
              <ShoppingBag :size="15" class="text-theme-accent" /> Return to Bookstore <ArrowRight :size="14" />
            </NuxtLink>
          </div>
        </div>
