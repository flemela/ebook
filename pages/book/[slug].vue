...
const activePdfFormat = computed<ProductFormat>(() => {
  ...
  const defaultEbookPrice = book.value?.price && book.value.price < 500 ? book.value.price : 149;
  return {
...
const pageDescription = computed(() => {
  ...
  return `${book.value?.name || 'Book'} ${authorText}${priceText}Instant Cloudflare R2 digital eBook PDF download at The Sunrise Bookstore.`;
});
...
<template>
  <div class="min-h-screen flex flex-col bg-theme-canvas text-theme-ink antialiased">
    ...
    <main class="max-w-5xl mx-auto w-full py-6 sm:py-10 px-4 sm:px-6 space-y-6 flex-1">
      ...
      <div v-if="error || !book" class="bg-theme-surface rounded-2xl border border-theme-border p-12 text-center space-y-3 shadow-subtle">
        <BookOpen :size="36" class="mx-auto text-theme-muted opacity-40" />
        <h2 class="font-display font-bold text-lg text-theme-ink">Book Not Found</h2>
        <p class="text-xs text-theme-muted">The requested title may have been unlisted or moved.</p>
        <NuxtLink to="/" class="inline-block text-xs font-bold text-theme-accent underline pt-2">Return to homepage</NuxtLink>
      </div>

      <!-- Main Book Viewport -->
      <div v-else class="bg-theme-surface rounded-2xl border border-theme-border p-6 sm:p-10 grid md:grid-cols-12 gap-8 lg:gap-12 shadow-card">
        <!-- Cover Art Frame -->
        <div class="md:col-span-5 flex justify-center items-start">
          <div class="w-full max-w-[280px] sm:max-w-[320px] aspect-[3/4] bg-theme-surface-subtle rounded-xl overflow-hidden shadow-medium border border-theme-border relative">
            <img
              :src="primaryImage"
            ...
            <div v-if="activePricing.discountPercentage > 0 || book.badge" class="absolute top-3 left-3 z-10 flex flex-col gap-1">
              <span v-if="activePricing.discountPercentage > 0" class="text-[9px] font-mono font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-theme-accent text-white shadow-sm">
                -{{ activePricing.discountPercentage }}% OFF
              </span>
              <span v-if="book.badge" class="text-[9px] font-mono font-bold uppercase tracking-wider px-2.5 py-0.5 rounded bg-theme-dark text-white shadow-sm">
                ★ {{ book.badge }}
              </span>
            </div>
          </div>
        ...
        <div class="md:col-span-7 space-y-5 text-left">
          <div class="space-y-1">
            ...
            <p class="text-xs text-theme-muted">
              By <strong class="text-theme-ink font-semibold">{{ book.author || 'Original Edition' }}</strong>
            </p>
          </div>
          ...
          <div class="pt-3 border-t border-theme-border space-y-2">
            ...
            <div class="p-4 rounded-xl border border-theme-accent-border bg-theme-accent-soft flex items-center justify-between">
              <div class="space-y-1">
                <div class="flex items-center gap-1.5 text-xs font-bold text-theme-accent-hover uppercase">
                  <Download :size="14" class="text-theme-accent" />
                  <span>Digital eBook (PDF Edition)</span>
                </div>
                <span class="text-[11px] text-theme-muted block font-mono">
                  {{ formatFileSize(activePdfFormat.file_size_bytes) }} • Instant Token Delivery
                </span>
              </div>
              ...
              <div class="text-right font-mono">
                ...
                <span
                  v-if="activePricing.originalPrice && activePricing.originalPrice > activePricing.currentPrice"
          ...
          <div class="pt-2">
            ...
            >
              <ShoppingBag :size="16" />
              <span>
                Add eBook to Cart • {{ formatCurrency(activePricing.currentPrice) }}
              </span>
            </button>
          </div>
          ...
          <div class="flex items-center gap-2 text-xs text-theme-ink pt-1">
            <CheckCircle2 :size="15" class="text-theme-accent flex-shrink-0" />
            <span>
              Instant Cloudflare R2 download tokens &amp; permanent email backup issued upon M-Pesa approval.
            </span>
          </div>

          <!-- Description -->
          <div v-if="book.description" class="pt-4 border-t border-theme-border space-y-1.5 text-xs text-theme-muted leading-relaxed" v-html="book.description" />
        </div>
      </div>
    </main>
