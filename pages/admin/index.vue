...
<template>
	<AdminLayout>
		<div class="space-y-7 max-w-6xl mx-auto">
			...
			<!-- KPI Metrics Grid (4 Dynamic Cards) -->
			<div class="grid grid-cols-2 lg:grid-cols-4 gap-4">

				<!-- Metric 1: Total Titles in Catalog -->
				<div
					class="bg-paper-surface p-5 sm:p-6 rounded-2xl border border-paper-border shadow-soft space-y-2 hover:shadow-card transition-shadow">
					<div class="flex justify-between items-center text-ink-subtle">
						...
						<div
							class="w-8 h-8 rounded-xl bg-forest-950/5 text-forest-900 flex items-center justify-center">
							<BookOpen :size="16" />
						</div>
					</div>
					...
					<span class="text-[11px] text-ink-muted block">Live database titles</span>
				</div>
				...
				<div
					...
					<div class="flex justify-between items-center text-ink-subtle">
						...
						<div
							class="w-8 h-8 rounded-xl bg-emerald-500/10 text-emerald-800 flex items-center justify-center">
							<Inbox :size="16" />
						</div>
					</div>
					<p class="text-2xl sm:text-3xl font-mono font-extrabold text-forest-950 tabular-figure">
				...
				<div
					...
					<p
						class="text-xl sm:text-2xl lg:text-3xl font-mono font-extrabold text-forest-950 tabular-figure truncate">
						{{ formatCurrency(stats?.todayRevenue ?? 0) }}
					</p>
					<span class="text-[11px] text-ink-muted block">Settled M-Pesa receipts</span>
				</div>
			</div>

			<!-- Quick Action Navigation Tiles -->
			<div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
				<NuxtLink to="/admin/books/import"
					...
					<div class="space-y-1 text-left">
						<span class="text-[9px] font-mono uppercase font-bold tracking-wider text-gold-300">Catalog
							Pipeline</span>
					...
					<div
						class="w-11 h-11 rounded-xl bg-white/10 flex items-center justify-center text-gold-300 flex-shrink-0 ml-3 group-hover:scale-105 transition-transform">
				...
				<NuxtLink to="/admin/books"
					...
					<div class="space-y-1 text-left">
						<span
							class="text-[9px] font-mono uppercase font-bold tracking-wider text-ink-subtle">Inventory</span>
						<h3 class="font-display font-bold text-sm sm:text-base text-forest-950">Manage Catalog</h3>
						<p class="text-[11px] text-ink-muted leading-normal">Set formats, prices, and PDF keys</p>
					</div>
					<div
						class="w-11 h-11 rounded-xl bg-paper-cream flex items-center justify-center text-forest-900 flex-shrink-0 ml-3 group-hover:scale-105 transition-transform">
						<BookOpen :size="20" />
					</div>
				</NuxtLink>
				...
				<NuxtLink to="/" target="_blank"
					class="bg-paper-surface p-5 sm:p-6 rounded-2xl border border-paper-border hover:border-forest-800/30 shadow-soft hover:shadow-medium hover:-translate-y-0.5 transition-all flex items-center justify-between group">
					<div class="space-y-1 text-left">
						<span class="text-[9px] font-mono uppercase font-bold tracking-wider text-gold-600">Reader
							Experience</span>
					...
					<div
						class="w-11 h-11 rounded-xl bg-paper-cream flex items-center justify-center text-gold-600 flex-shrink-0 ml-3 group-hover:scale-105 transition-transform">
						<ExternalLink :size="18" />
					</div>
				</NuxtLink>
			</div>

			<!-- Recent Orders Table & Real-Time Action -->
			<div class="bg-paper-surface rounded-2xl border border-paper-border shadow-soft overflow-hidden">
				<div class="px-5 py-4 border-b border-paper-border flex justify-between items-center bg-paper-cream/30">
					<h3 class="font-display font-bold text-sm sm:text-base text-forest-950">Recent Customer Orders</h3>
					<NuxtLink to="/admin/orders"
						class="text-xs font-bold text-forest-900 hover:text-gold-600 flex items-center gap-1">
						<span>View All in Orders Desk</span>
				...
				<div class="overflow-x-auto">
					<table class="w-full text-left text-xs border-collapse">
						<thead>
							<tr
								class="border-b border-paper-border text-ink-subtle uppercase tracking-wider font-mono text-[9px]">
								...
								<th class="py-3 px-5 font-semibold">Payment Status</th>
						...
						<tbody class="divide-y divide-paper-border/60">
							...
							<tr v-for="order in stats?.recentOrders" :key="order.id"
								...
								<td
									class="py-3.5 px-5 text-right font-mono font-bold text-forest-950 text-xs tabular-figure">
								...
								<td class="py-3.5 px-5">
									<span
										...
										:class="{
                      'bg-amber-50 text-amber-900 border-amber-200': order.paymentStatus === 'pending',
                      'bg-emerald-50 text-emerald-900 border-emerald-200': order.paymentStatus === 'paid',
                      'bg-red-50 text-red-900 border-red-200': order.paymentStatus === 'failed',
                    }">
										<span class="w-1.5 h-1.5 rounded-full flex-shrink-0" :class="{
                        'bg-amber-600': order.paymentStatus === 'pending',
                        'bg-emerald-600': order.paymentStatus === 'paid',
                        'bg-red-600': order.paymentStatus === 'failed',
                      }" />
										{{ order.paymentStatus === 'paid' ? 'Paid & Unlocked' : order.paymentStatus }}
									</span>
								</td>
								...
								<td class="py-3.5 px-5 text-right">
									<button v-if="order.paymentStatus === 'pending' || order.status === 'pending'"
										type="button"
										...
										<span>{{ confirmingOrderId === order.id ? 'Releasing...' : 'Approve & Release'
											}}</span>
									</button>
